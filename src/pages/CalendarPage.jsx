...
const CalendarPage = () => {
  ...
  const matchedMonth = useMemo(() => {
    ...
    const found = months.find((m) => {
      ...
      if (yearMatch && !indexMatch) {
        console.log(
          `⚠️ Year matches but index doesn't: month.bs_month_index=${m.bs_month_index}, view.month=${view.month}`,
        );
      }
    ...
    if (!found) {
      console.warn(
        `⚠️ No month found for year ${currentYear.year_label} (${currentYear.id}) with month index ${view.month}`,
      );
      console.log(
        "Available months:",
        months.filter((m) => m.year_id === currentYear.id),
      );
    } else {
  ...
  const fetchCalendarDays = useCallback(
    async (monthId) => {
      ...
      try {
        ...
        const iMap = {};
        days.forEach((d) => {
          tMap[d.day_number] = d.day_type
            ? { type: d.day_type, category: d.category_name || "" }
            : null;
  ...
  /* ── navigation ── */
  const goToday = () =>
    setView({ year: todayBs?.year ?? 2083, month: todayBs?.month ?? 1 });

  const refAd = bsToAd(view.year, view.month, 15);
  const adLabel = refAd
    ? `${AD_MONTHS[refAd.getMonth()]} ${refAd.getFullYear()}`
    : "";
  ...
  const isToday = (y, m, d) =>
    todayBs && y === todayBs.year && m === todayBs.month && d === todayBs.day;
  ...
  const handleCellClick = (bsDay, isCurrent) => {
    ...
    if (!dayId)
      return toast.error(
        "This day has not been generated in the database yet.",
      );

    const dayData = dayTypeMap[bsDay];
    const currentTypeName = dayData ? dayData.type : null;
    const currentTypeObj = currentTypeName
      ? availableDayTypes.find((t) => t.day_type === currentTypeName)
      : null;
      ...
      <div className="flex items-center gap-3 text-xs text-slate-500 flex-wrap">
        {matchedMonth ? (
          <>
            ...
            {Object.entries(stats).map(([type, count]) => {
              ...
              return (
                <span
                  key={type}
                  className={`px-2.5 py-1 rounded-full border ${s.badge}`}
                >
                  {count} {type}
                </span>
              );
          ...
          <span className="flex items-center gap-1">
            <AlertCircle size={11} />
            {!currentYear
              ? "No current year set in Settings → Calendar"
              : months.length === 0
                ? "API unavailable — grid view only"
                : "No matching month in DB for this view"}
          </span>
        )}
        {apiError && (
