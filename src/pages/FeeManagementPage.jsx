...
const FeeManagementPage = () => {
  ...
  const handleAddCategory = async (e) => {
    e.preventDefault();
    try {
      const res = await api.post("/v1/fees/categories", {
        name: newCatName,
        description: "",
      });
      if (res.data.success) {
  ...
  const searchStudents = async (value) => {
    ...
    try {
      const res = await api.get("/v1/fees/students/lookup", {
  ...
  const selectStudent = async (student) => {
    setStudentSearch(
      `${student.full_name} (${student.admission_no || student.id})`,
    );
    setStudentMatches([]);
    ...
    try {
      ...
      setStudentDue(null);
      toast.error("Unable to load student dues");
    }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm opacity-70 mt-1">
            Manage structures, collections, and receipts
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white/5 backdrop-blur-lg border border-white/10 p-6 rounded-2xl shadow-xl hover:border-indigo-500/50 transition-all">
              ...
              <h3 className="text-3xl font-bold mb-1">
                ₹{Number(stats.dailyCollection).toLocaleString()}
              </h3>
            ...
            <div className="bg-white/5 backdrop-blur-lg border border-white/10 p-6 rounded-2xl shadow-xl hover:border-blue-500/50 transition-all">
              ...
              <h3 className="text-3xl font-bold mb-1">
                ₹{Number(stats.totalExpected).toLocaleString()}
              </h3>
                  ...
                  {studentMatches.map((student) => (
                    ...
                    >
                      ...
                      <span className="block text-xs opacity-60">
                        {student.admission_no || `ID ${student.id}`} ·{" "}
              ...
              <div>
                {studentDue && (
                  <div className="mb-3 rounded-xl bg-white/5 border border-white/10 p-3 text-sm">
                    ...
                    <div className="text-xs opacity-60">
                      ...
                      {studentDue.section_name
                        ? ` / ${studentDue.section_name}`
                        : ""}{" "}
                      · Outstanding: ₹
                      {Number(
                        studentDue.totals?.outstanding || 0,
                      ).toLocaleString()}
                    </div>
                  </div>
                )}
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const fee = (studentDue?.dues || studentFees).find(
                      (f) => f.id == id,
                    );
                    setPaymentData({
                      ...paymentData,
                ...
                >
                  ...
                  {(
                    ...
                    studentFees.filter(
                      (f) =>
                        f.student_id == paymentData.student_id &&
                        f.status !== "paid",
                    )
                  ).map((f) => (
                    <option key={f.id} value={f.id}>
                      {f.fee_category_name || f.category} - Balance: ₹
                      {f.balance}
                    </option>
                  ))}
                </select>
